...
- Node.js 24 (see `.nvmrc`) — the CI builds on this version, and the committed
  `package-lock.json` is generated by the npm 11 that ships with it. Older Node
  versions still build, but `npm ci` will fail and `npm install` will rewrite the
  lockfile. This applies to developing the node only; the published package sets
  no `engines` constraint and runs on any Node version your n8n release supports.
- npm
